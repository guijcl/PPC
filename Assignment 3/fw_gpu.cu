...
#define BLOCKS (GRAPH_SIZE + THREADS_PER_BLOCK - 1)/THREADS_PER_BLOCK
...
__global__ void floyd_warshall_kernel(int graph_size, int *output, int k) {
    //WITH NO BLOCK SYNC BESIDES MULTIPLE KERNEL CALLS
    int i = blockIdx.x;
    int j = threadIdx.x;
    
    int thread_n = blockIdx.x * blockDim.x + threadIdx.x;
    if(thread_n >= graph_size) return;
    while(i < graph_size) {
      while(j < graph_size) {
